...
export function useEvents(limit = 100, level?: EventLevel, plane?: 'mcp' | 'cli') {
  const params = new URLSearchParams({ limit: String(limit) });
  if (level) params.set('level', level);
  if (plane) params.set('plane', plane);
  return useQuery({
    queryKey: ['events', limit, level, plane],
...
export function useCreateAccessKey() {
  ...
  return useMutation({
    mutationFn: (name: string) =>
      api.post<ApiKeyRecord & { secret?: string }>('/api/v1/access-keys', { name }),
    onSuccess: () => {
...
export function useSetProjection(id: string) {
  ...
  return useMutation({
    mutationFn: (input: {
      ...
      overrides?: { tool: string; visibility: OverrideVisibility }[];
...
export function useCallSeries(filter: Record<string, string | undefined>, bucket: string) {
